...
public class StaffListener implements Listener {
		...
		if (event.getPlayer().hasPermission("proxy.watermc.staff.join")) {
			for (ProxiedPlayer staff : Staff.getInstance().getProxy().getPlayers()) {
				if (staff.hasPermission("proxy.watermc.staff.join")) {
					staff.sendMessage(CC.translate("&9[S] " + CC.getNameColor(event.getPlayer()) + " &ajoined &7(&b" + event.getTarget().getName() + "&7)"));
				}
			}
		}
	}
	...
	public void onServerSwitch(ServerSwitchEvent event) {
		if (event.getPlayer().hasPermission("proxy.watermc.staff.join")) {
			for (ProxiedPlayer staff : Staff.getInstance().getProxy().getPlayers()) {
				if (staff.hasPermission("proxy.watermc.staff.join")) {
					staff.sendMessage(CC.translate("&9[S] " + CC.getNameColor(event.getPlayer()) + " &ajoined &7(&b" + event.getPlayer().getServer().getInfo().getName() + "&7)" + "&a from &7(&b" + event.getFrom().getName() + "&7)"));
				}
			}
		}
	}
		...
		if (event.getPlayer().hasPermission("proxy.watermc.staff.leave")) {
			for (ProxiedPlayer staff : Staff.getInstance().getProxy().getPlayers()) {
				if (staff.hasPermission("proxy.watermc.staff.leave")) {
					staff.sendMessage(CC.translate("&9[S] " + CC.getNameColor(event.getPlayer()) + " &cleft the network from &7(&b" + event.getPlayer().getServer().getInfo().getName() + "&7)"));
				}
			}
		}
	}
